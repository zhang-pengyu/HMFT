...
class ETCOM_comb2(BaseTracker):
    ...
    def apply_LF(self,scores_dis,scores_comp,feat_dis,feat_comp):
        return self.params.features.features[0].net.late_fusion(scores_dis,scores_comp,feat_dis,feat_comp)[0]
    ...
    def initialize(self, image1, image2, state, *args, **kwargs):
        ...
        # Initialize features
        self.initialize_features() # 初始化网络
...
#        self.im = im
        ...
        test_x_all = self.extract_sample(im, self.pos, sample_scales, self.img_sample_sz)
        
        test_x = test_x_all[0][0]
        text_x_vi = test_x_all[0][1]
        
        # Compute scores
        scores_dis, scores_comp = self.apply_filter(test_x,text_x_vi)
        scores_raw = self.apply_LF(scores_dis, scores_comp, self.params.features.features[0].feat_dis,self.params.features.features[0].feat_comp)
        translation_vec, scale_ind, s, flag = self.localize_advanced(scores_raw)
        ...
        if getattr(self.params, 'update_classifier', False) and update_flag:
            ...
            train_x_vi = TensorList([x[scale_ind:scale_ind+1, ...] for x in text_x_vi])
            ...
            self.update_classifier_vi(train_x_vi, train_y, target_box, learning_rate, s[scale_ind,...])
    ...
    def apply_filter(self, sample_x: TensorList,sample_x_vi: TensorList):
        with torch.no_grad():
            scores_dis = self.target_classifier.classify(self.target_filter, sample_x[0])
            scores_comp = self.target_classifier_vi.classify(self.target_filter_vi, sample_x_vi[0])

        return scores_dis[...,:sample_x[0].shape[-2],:sample_x[0].shape[-1]], scores_comp[...,:sample_x[0].shape[-2],:sample_x[0].shape[-1]]
    ...
    def localize_advanced(self, scores):
        ...
        if self.target_sz[0].item() < 1 or math.isinf(self.target_sz[0].item()):
            self.target_sz[0] = 1
        if self.target_sz[1].item() < 1 or math.isinf(self.target_sz[1].item()):
            self.target_sz[1] = 1
    ...
    def extract_sample(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        return self.params.features.extract_comb(im, pos, scales, sz)
    ...
    def generate_init_samples(self, im: torch.Tensor) -> TensorList:
        ...
        init_samples, init_samples_vi = self.params.features.extract_transformed_comb(im, self.pos, self.target_scale, aug_expansion_sz, self.transforms)

        # Remove augmented samples for those that shall not have
        for i, use_aug in enumerate(self.fparams.attribute('use_augmentation')):
            if not use_aug:
                init_samples[i] = init_samples[i][0:1, ...]
                init_samples_vi[i] = init_samples_vi[i][0:1, ...]
        ...
        if 'dropout' in self.params.augmentation:
            ...
            for i, use_aug in enumerate(self.fparams.attribute('use_augmentation')):
                if use_aug:
                    ...
                    init_samples_vi[i] = torch.cat([init_samples_vi[i], F.dropout2d(init_samples_vi[i][0:1,...].expand(num,-1,-1,-1), p=prob, training=True)])

        return init_samples, init_samples_vi
    ...
    def init_classifier_vi(self, x):
        # Get target classifier network
        self.target_classifier_vi = self.params.features.get_unique_attribute('target_classifier_vi')
                                                                            
        # Get target boxes for the different augmentations
        target_boxes = self.init_target_boxes()

        self.classifier_net_name = type(self.target_classifier_vi).__name__

        if 'Simple' in self.classifier_net_name:
            with torch.no_grad():
                target_filter = self.target_classifier_vi.get_modulation(x[0], target_boxes)
            self.target_filter_vi = target_filter.detach().mean(dim=0, keepdim=True)
            ...
            if hasattr(self.params, 'num_neg_clusters'):
                self.target_classifier_vi.num_neg_clusters = self.params.num_neg_clusters

            if hasattr(self.params, 'num_cluster_iter'):
                self.target_classifier_vi.cluster_iter = self.params.num_cluster_iter

            if hasattr(self.params, 'use_init_filter'):
                self.target_classifier_vi.filter_optimizer.use_init_filter = self.params.use_init_filter
            ...
            with torch.no_grad():
                self.target_filter_vi, losses = self.target_classifier_vi.get_filter(x[0], target_boxes, y[0], num_iter=num_iter,
                                                                                   compute_losses=plot_loss, train_bb=target_boxes)

            # TODO do this better
            self.target_classifier_vi.filter_optimizer.use_init_filter = True
    ...
    def update_classifier_vi(self, train_x, train_y, target_box, learning_rate=None, scores=None):
        hard_negative_flag = learning_rate is not None
        if learning_rate is None:
            learning_rate = self.fparams[0].learning_rate
        init_samples_weight = getattr(self.fparams[0], 'init_samples_minimum_weight', 0.0)
        ...
        if 'Simple' in self.classifier_net_name:
            ...
            if num_iter != 0:
                ...
                sample_weights = self.sample_weights[0][:self.num_stored_samples[0]]
                with torch.no_grad():
                    self.target_filter_vi, losses = self.target_classifier_vi.filter_optimizer(self.target_filter_vi, samples, labels,
                                                                                         num_iter=num_iter, compute_losses=plot_loss,
                                                                                         train_bb=target_boxes,
                                                                                         sample_weight=sample_weights)
            ...
            if not hasattr(self, 'init_target_filter_vi'):
                self.init_target_filter_vi = self.target_filter_vi

            num_iter = getattr(self.params, 'net_opt_update_iter', None)
            target_box = target_box.to(train_x[0].device)
            with torch.no_grad():
                target_filter_update_vi, losses = self.target_filter_vi.get_modulation(train_x[0], target_box, train_y[0],
                                                                                     num_iter=num_iter,
                                                                                     compute_losses=False, train_bb=target_box)
            self.target_filter_vi = init_samples_weight * self.init_target_filter_vi + (1 - init_samples_weight) * \
                                    ((1 - learning_rate) * self.target_filter_vi + learning_rate * target_filter_update_vi)
    ...
    def track_sequence(self, sequence):
        # Initialize
        rgb_image = self._read_image(sequence['rgb_frame'][0])
        t_image = self._read_image(sequence['t_frame'][0])

        times = []
        start_time = time.time()
        self.initialize(rgb_image, t_image, sequence['rgb_gt'][0,:])
        init_time = getattr(self, 'time', time.time() - start_time)
        times.append(init_time)

        if self.params.visualization:
            self.init_visualization()
            self.visualize(rgb_image, sequence['rgb_gt'][0,:])

        # Track
        tracked_bb = [sequence['rgb_gt'][0,:]]
        frame_number = 1
        seq_len = len(sequence['rgb_frame'])
        for frame in range(1,seq_len):
            # Set gt pos
            if frame_number < sequence['rgb_gt'].shape[0]:
                gt_state = sequence['rgb_gt'][frame_number, :]
            ...
            rgb_image = self._read_image(sequence['rgb_frame'][frame])
            t_image = self._read_image(sequence['t_frame'][frame])

            start_time = time.time()
            state = self.track(rgb_image,t_image)
            ...
            if self.params.visualization:
                self.visualize(rgb_image, state)
