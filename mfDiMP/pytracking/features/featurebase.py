...
class MultiFeatureBase(FeatureBase):
    ...
    def get_feature_comb(self, im: torch.Tensor):
        """Get the feature. Generally, call this function.
        args:
            im: image patch as a torch.Tensor.
        """
        ...
        feat_list, feat_list_vi = self.extract_comb(im)
        ...
        output_sz = [None]*len(feat_list_vi) if self.output_size is None else self.output_size

        # Pool/downsample
        for i, (sz, s) in enumerate(zip(output_sz, self.pool_stride)):
            if sz is not None:
                feat_list_vi[i] = F.adaptive_avg_pool2d(feat_list_vi[i], sz)
            elif s != 1:
                feat_list_vi[i] = F.avg_pool2d(feat_list_vi[i], s, s)

        # Normalize
        if self.normalize_power is not None:
            for feat in feat_list_vi:
                feat /= (torch.sum(feat.abs().view(feat.shape[0],1,1,-1)**self.normalize_power, dim=3, keepdim=True) /
                         (feat.shape[1]*feat.shape[2]*feat.shape[3]) + 1e-10)**(1/self.normalize_power)

        return feat_list, feat_list_vi
    
    def get_feature_comb_vi(self, im: torch.Tensor):
        """Get the feature. Generally, call this function.
        args:
            im: image patch as a torch.Tensor.
        """
        ...
        feat_list = self.extract_comb_vi(im)
