...
def sample_patch(im: torch.Tensor, pos: torch.Tensor, sample_sz: torch.Tensor, output_sz: torch.Tensor = None):
    ...
    if math.isinf(sample_sz[0].item()):
        sample_sz[0] = im.shape[2]
        sample_sz[1] = im.shape[3]
    ...
    if df > 1:
        ...
        im2 = im[..., min(os[0].item(),im.shape[2]-2)::df, min(os[1].item(),im.shape[3]-2)::df]   # downsample
    else:
        im2 = im
    ...
    # Get image patch
    im_patch = F.pad(im2, (int(-tl[1].item()), int(br[1].item()) - im2.shape[3] + 1, int(-tl[0].item()), int(br[0].item()) - im2.shape[2] + 1), 'replicate')
