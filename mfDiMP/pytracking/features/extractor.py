...
class MultiResolutionExtractor(ExtractorBase):
    ...
    def extract_comb(self, im, pos, scales, image_sz):
        """Extract features.
        args:
            im: Image.
            pos: Center position for extraction.
        ...
        # Compute features
        feature_map = TensorList([f.get_feature_comb(im_patches) for f in self.features]).unroll()
    ...
    def extract_transformed_comb(self, im, pos, scale, image_sz, transforms):
        """Extract features from a set of transformed image samples.
        args:
            im: Image.
            pos: Center position for extraction.
        ...
        # Compute features
        feature_map = TensorList([f.get_feature_comb(im_patches)[0] for f in self.features]).unroll()
        feature_map_vi = TensorList([f.get_feature_comb(im_patches)[1] for f in self.features]).unroll()
        return feature_map, feature_map_vi

    def extract_transformed_comb_vi(self, im, pos, scale, image_sz, transforms):
        """Extract features from a set of transformed image samples.
        args:
            im: Image.
            pos: Center position for extraction.
        ...
        # Compute features
        feature_map = TensorList([f.get_feature_comb_vi(im_patches) for f in self.features]).unroll()
